...
function RadarrWidget({ config }: WidgetProps<RadarrConfig>) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [statusRes, moviesRes, queueRes, missingRes] = await Promise.all([
          arrGet(url, apiKey, '/api/v3/system/status'),
          arrGet(url, apiKey, '/api/v3/movie'),
        ...
        setStats({
          ...
          movies:  (moviesRes as unknown[]).length,
    ...
    <div className="widget-body" style={{ padding: '12px 14px', gap: 10 }}>
      ...
      {!url || !apiKey ? (
        <div style={{ color: 'var(--text2)', fontSize: 12, lineHeight: 1.5 }}>
          Configure a Radarr URL and API key in widget settings.
        </div>
      ) : loading ? (
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {[
            { label: 'Movies',  value: stats.movies,  color: 'var(--accent)' },
